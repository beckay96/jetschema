...
import { Database, Table, Settings, Zap, Code, List, HardDrive } from 'lucide-react';
...
export function TabsDropdown({ activeTab, onTabChange }: TabsDropdownProps) {
  ...
  const tabIcons: Record<string, JSX.Element> = {
    ...
    enumerations: <List className="h-4 w-4" />,
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-[200px]">
        ...
        <DropdownMenuItem onSelect={() => onTabChange("enumerations")}>
          <List className="h-4 w-4 mr-2" />
          <span>Enumerations</span>
        </DropdownMenuItem>
        <DropdownMenuItem onSelect={() => onTabChange("storage")}>
