  ...
  return <div className="min-h-screen bg-gradient-to-br from-background to-muted/20">
        ...
        <div className="px-[5px] py-0 rounded-sm">
          <div className="flex items-center justify-center">
            ...
            <div className="flex-row ">
              ...
              {isMobile && <>
                  <Button variant="outline" size="sm" onClick={toggleLeftPanel} className="lg:hidden">
                    <PanelLeft className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="sm" onClick={toggleRightPanel} className="lg:hidden">
      ...
      <div className="flex h-[calc(100vh-80px)] relative">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {!isMobile && <div className="absolute top-4 left-4 z-10 flex gap-2">
              <Button variant="outline" size="sm" onClick={toggleLeftPanel} className={leftPanelOpen ? 'bg-muted' : ''}>
                <PanelLeft className="h-4 w-4" />
              </Button>
            </div>}
          
          {!isMobile && <div className="absolute top-4 right-4 z-10 flex gap-2">
          ...
          <div className="flex-1">
            <DatabaseCanvas tables={tables} onTableUpdate={setTables} onTableSelect={setSelectedTable} selectedTable={selectedTable} />
          </div>
        </div>
        ...
        `}>
          ...
          {isMobile && rightPanelOpen && <div className="absolute top-4 left-4 z-10">
              <Button variant="ghost" size="sm" onClick={() => setRightPanelOpen(false)} className="border border-gray-500 mx-0 my-[4px]">
                <X className="h-4 w-4" />
              </Button>
            </div>}
          ...
          <Tabs defaultValue="sql" className="h-full flex flex-col">
            ...
            <TabsContent value="properties" className="flex-1 p-4 pt-2">
              <Card className="h-full">
                <div className="p-4">
                  {selectedTable ? <div className="space-y-4">
                      <div>
        ...
        {/* Mobile backdrop */}
        {isMobile && (leftPanelOpen || rightPanelOpen) && <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => {
        setLeftPanelOpen(false);
        setRightPanelOpen(false);
      }} />}
      </div>
